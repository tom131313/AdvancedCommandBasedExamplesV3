...
public class AchieveHueGoal implements Mechanism {
   ...
   */
  public AchieveHueGoal(LEDView robotSignals, BiConsumer<Runnable, Double> RobotAddPeriodic, DoubleSupplier hueSetpoint) {
    ...
    m_robotSignals.setSignal(m_notSeekingGoalSignal);
     ...
     // The {@link #reset()} could stop the controller but since this is always getting a measurement
     // and setpoint it keeps calculating.
    RobotAddPeriodic.accept(controllerCalculation(), HueGoal.LOOP_SPEED.in(Seconds));
  }
   ...
   * indicated and this display command stops. (But the PID controller would still be running but
   * its results are being ignored if this command is not running.)
   * 
   * @return command used to display the action of the PID controller
   */
  public Command achieveHue() {
    return
        run(coroutine ->
            {
              // initialize display and the PID controller
              LEDPattern m_currentStateSignal = m_notSeekingGoalSignal; // soon but not this instant
              reset();

              // run display of progress to the setpoint
              ...
              // set LED pattern to blink to show controller at setpoint                
  ...
  public void reset() {
    ...
    m_hueController.reset(); // tries to stop controller but it's passed a measurement and setpoint so it runs anyways
                    // could add a "run" switch to the {@link #controllerCalculation()} but wasn't done in this example
    m_currentStateHue = 0.; // also considered the initial and previous state ( 0 is arbitrary - not indicative of anything)
    m_robotSignals.setSignal(m_notSeekingGoalSignal);
  }
