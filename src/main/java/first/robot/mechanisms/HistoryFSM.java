...
import first.robot.Constants.HueHistory;
...
public class HistoryFSM implements Mechanism {
  ...
  public HistoryFSM(LEDView robotSignals, BiConsumer<Runnable, Double> RobotAddPeriodic) {
        ...
        coroutine -> 
          {
            ...
            while(true)
            {
              ...
              coroutine.wait(HueHistory.CHANGE_COLOR_PERIOD); // display color for this long
            }
          }
   ...
   *
   * <p>Set a random color that hasn't been used in the last "HueHistory.LOCKOUT_COLOR_PERIOD"
   */
  private void getHSV() {
    ...
    do {
      ...
      if (colorTime.lt(currentTime.minus(HueHistory.LOCKOUT_COLOR_PERIOD))) {
