...
public abstract class CommandsTriggers {
  ...
  private static final Command firstJob = Command.noRequirements(_ ->
      {
  ...
  private static final Command secondJob = Command.noRequirements(_ ->
...
/**
  ...
  public static Command getFirstCommandTriggersNextTest() {
    ...
    else
    {
      return Command.noRequirements(_ ->
          Alerts.TRIGGER_NEXT_COMMAND.set(true)).named("Alert-getFirstCommandTriggersNextTest");
    }
  }
  ...
  public static Command lightBar() {
    if (m_lightBar.isPresent()) {
      ...
      return Command.noRequirements(_ ->
          Alerts.MOORE_FSM_LIGHTBAR.set(true)).named("Alert-lightBar");
    }
  }
...
/**
  ...
  public static Command setAutonomousSignal() {
    if (m_UseAutonomousSignal.isPresent()) {
      ...
      return Command.noRequirements(_ ->
           Alerts.AUTONOMOUS_SIGNAL.set(true)).named("Alert-AutonomousSignal");
    }
  }
  ...
  private static void configureGameControllersBindings() {
    ...
    {  
      ...
      // pressing X starts the color wheel progression then it runs automatically from the LEDPattern
      // being dynamic and running in the Mechanism background. Pressing X repeatedly does start the
      // command with each press but you can't see the color change because the color is based on
      // the clock time and the clock time hasn't changed (much, if at all).
    ...
    if (m_achieveHueGoal.isPresent())
      ...
      Alerts.ACHIEVE_HUE_GOAL.set(true);
    }
  }
